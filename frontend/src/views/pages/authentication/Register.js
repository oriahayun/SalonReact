  ...
  const onSubmit = async data => {
    ...
    if (Object.values(tempData).every(field => field.length > 0) && data.terms === true) {
      ...
      try {
        ...
        const userData = { ...newUser, accessToken: user.accessToken, role: 'admin' }
        ability.update([{action: 'manage', subject: 'all'}])
        dispatch(handleLogin(userData))
        navigate('/')
        // Additional actions after successful registration (e.g. redirect to home page)
      } catch (error) {
    ...
    <div className='auth-wrapper auth-cover'>
      <Row className='auth-inner m-0'>
        <Link className='brand-logo' to='/' onClick={e => e.preventDefault()}>
          ...
          <h2 className='brand-text text-primary ms-1'>Salon</h2>
        </Link>
